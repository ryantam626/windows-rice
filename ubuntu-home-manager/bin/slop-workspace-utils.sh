...
ai() {
    ...
    local rel target_root target_parent

    local ai_slop_branch_name="rt.ai-slop"
    ...
    if [[ ! -d "$target_root" ]]; then
        echo "Creating slop worktree at $target_root"
        mkdir -p "$target_parent"
        git -C "$root" worktree add "$target_root" -b "$ai_slop_branch_name"|| return 1
    fi
